...
def generate_recommendations(
    ...
    # Calculate market price and expected metrics from competitor sales
    # Edge case handling: Check for null/zero values before division
    competitor_metrics = (
        competitor_sales
        .groupBy("item_id")
        .agg(
            F.avg("marketplace_price").alias("market_price"),
            F.sum("units_sold").alias("total_units_sold"),
            F.countDistinct("seller_id").alias("num_sellers")
        )
        .withColumn(
    ...
    # Join missing items with competitor metrics
    recommendations = missing_items.join(
        competitor_metrics,
        "item_id",
        "left"
    )
    ...
    recommendations = recommendations.withColumn(
        "expected_revenue",
        F.coalesce(F.col("expected_units_sold") * F.col("market_price"), F.lit(0.0))
    )
    
    # Fill nulls for items without competitor data
