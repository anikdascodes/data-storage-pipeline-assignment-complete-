"""
ETL Pipeline for Company Sales (FIXED VERSION)
...
def main(config_path: str):
    ...
    logger.info("Starting ETL pipeline for Company Sales (FIXED VERSION)")
    ...
    if 'source_path' in config['company_sales']:
        source_path = config['company_sales']['source_path']
        bronze_path = config['company_sales']['bronze_path']
        archive_path = config['company_sales']['archive_path']
    else:
        bronze_path = config['company_sales']['input_path']
        source_path = None
        archive_path = None

    hudi_output_path = config['company_sales']['hudi_output_path']
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="ETL Pipeline for Company Sales (FIXED)")
